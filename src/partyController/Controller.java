...
/* You will need to change and add some code in this class.
  ...
  Then you add code in this class according to instructions and finish the
  classes Address, Guest and GuestManager according to comments in those
  classes.
...
public class Controller {
  ...
  public Controller(int maxNbrOfGuests) {
    // Creates a GuestManager-object referenced by the instance variable
    // register
    register = new GuestManager(maxNbrOfGuests);

    // Create main GUI-object referenced by the instance variable view
    view = new MainFrame(this);
    // give the GUI a reference to the Controller-object by using this to send a
    // reference to the Controller-object

    // Update the GUI with information from the guest list (even if it might be
    // empty)
  ...
  public void buttonPressed(ButtonType button) {
     ...
     *
     * It is important to keep the list in GuestManager with information about
     * Guests
     * matched with what is shown in the GUI. Otherwise, it will not match when
     * we want to change or delete a guest from the list.
     */
   ...
   * list to the right in the GUI. It updates the information to the left in the
   * GUI
  ...
  private boolean validateIndex(int index) {
    boolean ok = true;
    if (index < 0) {
      JOptionPane.showMessageDialog(null, "Select a person in the list!");
      ok = false;
    }
    return ok;
  }
  ...
  private void setDefaultValuesInView() {
    view.setFirstNameText("");
    view.setLastNameText("");
    view.setAgeText("");
    view.setStreetText("");
    view.setZipCodeText("");
    view.setCityText("");
